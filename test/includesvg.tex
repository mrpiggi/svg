%\RequirePackage[ngerman=ngerman-x-latest]{hyphsubst}
% \documentclass[english,ngerman,BCOR=6mm,parskip=false]{tudscrreprt}
% \setlength{\parskip}{0.7ex}
\listfiles
\documentclass[parskip=full]{scrreprt}% Test
%\usepackage{selinput}\SelectInputMappings{adieresis={ä},germandbls={ß}}
\usepackage[T1]{fontenc}
%\usepackage{babel}

\usepackage[usexcolor=true,usetransparent=false,draft=false]{svg-new}
\usepackage[extract=true,clean=true,exclude=false]{svg-extract}

\svgsetup{lastpage=auto,convertformat=,convert=gs}

\svghidepreamblestart
%\usepackage{showframe}

\usepackage{blindtext}

\makeatletter
\setlength{\@fptop}{0pt}
\setlength{\@fpsep}{0pt}% twocolumn
\makeatother

\pdfsuppresswarningpagegroup=1%

\parskip=0pt
\parindent=0pt

\svghidepreambleend

\usepackage{amsmath}

%\RequirePackage{luatex85}[2016/06/15]% standalone
\begin{document}


%\begin{equation}
%a^2+b^2=c^2\label{eq:emc1}
%\end{equation}
\begin{equation}
a^2+b^2=c^2\label{eq:emc2}
\end{equation}
%\blinddocument

\svgpath{{svg-test/}{"C:/Users/Hanisch/Documents/Dateien/svg"}}

%\includeinkscape[]{/svg-out/Seriell_svg-tex.pdf}

%\newincludesvg[]{"Saeriell".svg}

%\includegraphics[]{S.bla.pdf}
%
%
%%\svgsetup{inkscapepath="C:/Program Files/Inkscape/inkscape.exe"}
%
%\makeatletter
%%\newcommand*\svgx@ifinlist[2]{%
%%  \begingroup%
%%    \def\svg@tempa##1,#1,##2\@nil{%
%%      \IfArgIsEmpty{##2}{%
%%        \aftergroup\@secondoftwo%
%%      }{%
%%        \aftergroup\@firstoftwo%
%%      }%
%%    }%
%%    \expandafter\svg@tempa\expandafter,#2,#1,\@nil
%%  \endgroup%
%%}%
%
%
%\meaning\svg@inkscape@exe
%\makeatother
%\svgpath{{"svg test/"}}%
%%\graphicspath{{foo/}}
%
%\blindtext
%
%%\svgsetup{inkscapeexe=ink}
%
%%\makeatletter
%%\filename@parse{svg/newimage-svg-extract.tex}
%%\filename@area,\filename@base,\filename@ext
%%\makeatother
%
%\begin{figure}
%\newincludesvg[%
%  width=307.28987pt,%
%  angle=30,%
%  svgpath="svg test",%
%%  extract=false,
%%  extractpath=out/,
%%  extractname=blubb,
%%  height=150pt,
%%  svgpath=bar,%
%%  scale=-1,
%%  dpi=300,
%%  inkscape={-C},%
%%  inkscape=300dai,%
%%  inkscape=aaadpi,%
%%  inkscape=page,%
%  inkscape=raw,
%%  dpi=50,
%%  inkscape=page,
%%  pretex=\Huge,
%%  inkscape=false,
%%  draft,%=false,
%]{"test image"}
%\end{figure}

%\graphicspath{{"svg-test"/}}
%\svgpath{{"svg test/"}{svgtest}}

%\makeatletter
%\let\input@path\Ginput@path
%\meaning\input@path
%\filename@parse{newimage-svg-raw.pdf}
%\meaning\filename@area\newline
%\meaning\filename@base\newline
%\meaning\filename@ext\newline
%\meaning\filename@area\newline
%\meaning\filename@base\newline
%\meaning\filename@ext\newline

%\svgsetup{%
%  inkscape=bla/inkscape.exe -z -C,%
%%  inkscapeexe="C:/Program Files/Inkscape/inkscape.exe"
%}

%\def\svgwidth{200pt}

\makeatletter

\svgsetup{pretex=\color{red}}

%\svgsetup{lastpage=false}

\fboxsep=-\fboxrule
%\begin{figure}
%\fbox{\includegraphics[width=\textwidth,angle=10]{svg-out/Seriell_svg-raw.png}}

%\end{figure}

%\def\svgwidth{400pt}
%\def\svgscale{0.6}

\begin{figure}
%\def\svgwidth{\textwidth}
%\def\svgscale{0.8}
\fbox{\newincludesvg[%
%  inkscapepath=basesubdir,%
%  exclude=true,
%  angle=10,%origin=c,
%  svgpath=C:/Users/Hanisch/Documents/Dateien/svg,
%  extractpreamble=test.aaa,
  clean=false,pdf=on,eps,png,
%  extractpreamble=ssss.tex,
%  inkscape=raw,
%  inkscape=eps,
  convertdensity=600x300,
%  convertdensity=555x300x,
%  angle=10,
%  inkscapedpi=,
%  outname="bla",
%  extractname=hamster,
%  lastpage=false,
%  draft,
%  extract=false,%
%  convertdpi=400dpi,
%  convertdpi={png+=700x300dpi},
%  convertdpi={jpg+=333dpix666dpi},
%  convert=true,%
%  lastpage=3,%
%  lastpage=false,%
%  convert=gs64,%
%  extractformat=,%{pdf,eps,ps},%
%  convertformat={png,tiff,jpg,eps},%
%  gsdevice={eps=\relax},
%  gsopt=bla,%
%%  gsopt={png=foo},%
%  gsopt={png+=bar},%
%  magicksetting=-depth 4,
%  magicksetting={png=-depth 8},
%  magickoperator=-resize 25\%,
%  magickoperator={png=-resize 25\%},
%  convertformat=png,
%  convertdensity=,%
  width=\textwidth,%
%  extractwidth=\relax,
%  extractwidth=400pt,
%  scale=1.1,
%  extractscale=1.2,
%  extractpretex=inherit,
%  extractwidth=,
%  clean=false,
%  angle=-180,
%  draft,
%  height=4cm,
%  scale=0.5pt,
%  pretex=aaa,
%  apptex=bbb,
]{"Seriell"}%
}%
\caption{aaa}
\end{figure}


\newbox\blabox
%\savebox\blabox{\newincludesvg[height=200pt]{Seriell}}

\usebox\blabox

\the\ht\blabox

\the\dp\blabox

\blindtext

\the\dimexpr 30pt*2\relax

%\begin{figure}
%%\newincludesvg[%
%%  width=200pt,%
%%%  path="svgtest",
%%%  extractname=bla blau,
%%%  angle=30,%
%%%  svgpath=svg test,%
%%]{testimage}
%\newincludesvg[%
%%  draft,
%%  extractname=bla,
%  width=.2\textwidth,%
%%  extractformat={eps},%
%%  angle=30,%
%%  clean=false,
%%  inkscape=raw,
%%  svgpath={"svg test/"},%
%]{newimage}
%\end{figure}

\def\bla{fafa}

\expandafter\ifx\csname bla\endcsname\@empty\relax
aaa
\else
nbbb
\fi

\expandafter\expandafter\expandafter\ifx\@nameuse{bla}\@empty\relax
aaa
\else
nbbb
\fi


\end{document}
